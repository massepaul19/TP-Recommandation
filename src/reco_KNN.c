...
// Variable globale pour stocker le recommandeur (nécessaire pour Predict sans paramètre rec)
static RecommandeurKNN* recommandeur_global = NULL;
...
RecommandeurKNN* initialiser_recommandeur_depuis_fichier(const char* filename) {
    RecommandeurKNN* rec = creer_recommandeur(5); // k=5 par défaut
    if (!rec) return NULL;
    ...
    for (int i = 0; i < rec->nb_transactions; i++) {
        ...
        if (!trouve && rec->nb_users < MAX_USERS) {
    ...
    for (int i = 0; i < rec->nb_transactions; i++) {
        ...
        if (!trouve && rec->nb_articles < MAX_ARTICLES) {
...
double calculer_prediction_ponderee(RecommandeurKNN* rec, Voisin* voisins, unsigned int id_article, unsigned int id_user) {
    ...
    for (int i = 0; voisins[i].index_utilisateur >= 0; i++) {
        ...
        // Vérifier si le voisin a noté cet article
    ...
    if (denominateur == 0.0) {
        return moyenne_user; // Retourner la moyenne de l'utilisateur par défaut
    }
    
    double prediction = moyenne_user + (numerateur / denominateur);
    
    // Borner la prédiction entre 1 et 5 (ou selon votre échelle)
